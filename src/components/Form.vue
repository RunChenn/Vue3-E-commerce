...
export default {
  ...
  setup(props, { emit }) {
    const validObj = yup.object({
      ...
      email: yup.string().nullable().required('必填').email('請輸入正確的email'),
      tel: yup.string().nullable().required('必填').min(8, '不能小於 8 個字元').max(10, '不能大於 10 個字元'),
...
<template>
  <form ref="form" @submit.prevent="createOrder">
    <p>顧客資訊</p>
    <div class="mb-3">
      <label for="email" class="form-label">Email</label>
      <input id="email" name="email" type="email" class="form-control" placeholder="請輸入 Email" rules="required|email" v-model="email" />
      <div class="text-danger">{{ errors.email }}</div>
    </div>

    <div class="mb-3">
      <label for="name" class="form-label">收件人姓名</label>
      <input id="name" name="姓名" type="text" class="form-control" placeholder="請輸入姓名" rules="required" v-model="name" />
      <div class="text-danger">{{ errors.name }}</div>
    </div>

    <div class="mb-3">
      <label for="tel" class="form-label">收件人電話</label>
      <input id="tel" name="電話" type="text" class="form-control" placeholder="請輸入電話" rules="required|min:8|max:10" v-model="tel" />
    ...
    <div class="mb-3">
      ...
      <input id="address" name="地址" type="text" class="form-control" placeholder="請輸入地址" rules="required" v-model="address" />
